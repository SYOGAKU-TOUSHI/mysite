	...
	static gen(gameData, seed) {
		...
		for (let y = 0; y < h; y ++) {
			for (let x = 0; x < w; x ++) {
				...
				const max = 10 + xors.random() % 50;		// 山の散布回数
		...
		for (let i = 0; i < size; i ++) {
			if (xors.random() %  6 === 0) { mapArr[i]= landIds.forest};	// 1/6で森
			if (xors.random() % 12 === 0) { mapArr[i]= landIds.water };	// 1/12で水
		}

		// ゲーム開始位置近辺は平地にする
		const xStrt = w / 2 | 0;	// 開始位置X
		const yStrt = h / 2 | 0;	// 開始位置Y
		for (let y = -1; y < 2; y ++) {			// y : -1～1
			for (let x = -1; x < 2; x ++) {		// x : -1～1
				const i = ((yStrt + y + h) % h) * w + ((xStrt + x + w) % w);
				mapArr[i] = landIds.plain;		// 3x3マスを平地で塗り潰し
			}
		}
